...
// limits 
VectorXd torque_limits(7);
VectorXd lower_joint_limits(7);
VectorXd upper_joint_limits(7);
...
int main() {
	...
	robot->_q << 0, -15, -15, -80, 0, 90, 0;
	robot->_q *= M_PI / 180;
	// robot->_q = VectorXd::Zero(7);
	// robot->_dq = VectorXd::Zero(7);
	robot->updateModel();

	robot->_rbdl_model->gravity = R.transpose() * Vector3d(0, 0, -9.81);
...
void simulation(Sai2Model::Sai2Model* robot, Simulation::Sai2Simulation* sim)
{
	...
	while (fSimulationRunning) {
		...
		// warning if command torques are over torques limits
		VectorXd total_torques = command_torques + g;
		for (int i = 0; i < 7; ++i) {
			if (std::abs(g(i)) > 0.9 * torque_limits(i)) {
				std::cout << "Torque violation on joint " << to_string(i + 1) << "\n";
				std::cout << last_time << "\n";
				std::cout << g(i) << "\n";
				// g(i) = 0.9 * torque_limits(i);
				g(i) = 0;
			}
		}
		...
		// warning if joint limits are out of range
		for (int i = 0; i < 7; ++i) {
			if (robot->_q(i) < lower_joint_limits(i)) {
				std::cout << "Lower joint violation on joint " << to_string(i + 1) << "\n";
				std::cout << last_time << "\n";
				robot->_q(i) = lower_joint_limits(i);
				robot->_dq(i) = 0;
			} else if (robot->_q(i) > upper_joint_limits(i)) {
				std::cout << "Upper joint violation on joint " << to_string(i + 1) << "\n";
				std::cout << last_time << "\n";
				robot->_q(i) = upper_joint_limits(i);
				robot->_dq(i) = 0;
			}
		}
